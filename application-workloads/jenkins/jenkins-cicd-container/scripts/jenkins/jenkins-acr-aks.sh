...
#install jenkins
run_util_script "scripts/jenkins/install_jenkins.sh" -jf "${jenkins_fqdn}" -cn "${cluster_name}" -spid "${service_principal_id}" -ss "${service_principal_secret}" -subid "${subscription_id}" -tid "${tenant_id}" -al "${artifacts_location}" -st "${artifacts_location_sas_token}"
